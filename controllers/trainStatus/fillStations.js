...
export async function trainStatusCron() {
  const trainDetails = await models.trainStatuses.findAll({
    where: {
      isRunning: 'true'
    }
  })
  ...
  cron.schedule('*/15 * * * *', async () => {
    for (let i = 0; i < trainIds.length; i++) {
      ...
      let start = await models.trainStatuses.findOne({
        where: {
          trainId: trainId
        }
      })
      ...
      if (now_city) {
        if (curr_city != now_city) {
          curr_city = now_city
          let temp1 = await models.statuses.findOne({
            where: {
              type: 'IN-TRANSIT'
            }
          })
          let inTransitId = temp1.dataValues.id
          let temp2 = await models.statuses.findOne({
            where: {
              type: 'COMPLETED'
            }
          })
          let completedId = temp2.dataValues.id
          const toBeUpdated = await models.packages.findAll({
            where: {
              trainId: trainId,
              dCity: curr_city,
          ...
          if (toBeUpdated.length > 0) {
            const updatedPackages = await models.packages.update({
      ...
      let inTransitId = temp1.dataValues.id
      let pastPackages = await models.packages.findAll({
        where: {
          statusId: inTransitId
        }
      })
      ...
      if (pastPackages) {
        for (let i = 0; i < pastPackages.length; i++) {
          ...
          let trainStatus = await models.trainStatuses.findOne({
            where: {
              id: trainStatusId
            }
          })
          if (trainStatus) {
            ...
            if (final < time) {
              ...
              let completedId = temp2.dataValues.id
              let changes = await models.packages.update({
                statusId: completedId
              }, {
                where: {
                  id: id
      ...
      let temp = await models.cities.findOne({
        where: {
          id: curr_city
        }
      })
...
export async function getCurrCity(trainId) {
  ...
  x = x.split('T')
  let time = x[0] + " " + x[1]
  let answer = await models.trainStatuses.findOne({
    where: {
      dTime: {
        [Op.gte]: time
      },
